...
$image = mysql_fetch_array(mysql_query("SELECT image FROM products WHERE productid = $id"));
$image = $image[0];
...
<?php
echo "<div align='center'><font color='red'><b>The item has been edited and appears as follows:</b></font><br><br><hr noshade size=1 width=800><br><br><table border='0' width='924'>
...
<!-- *************************************** -->
<input type="hidden" name="currency_code" value="USD">
<?php $result = mysql_query('SELECT * from sizeprice order by price');while($row = mysql_fetch_assoc($result))
{
$id = $row['id'];
$id = ($id-1);
$width = $row['width'];
...
<input type="hidden" name="cmd" value="_cart">
<input type="hidden" name="business" value="LJY46UVQUM4P8">
<br><input type="image" src="view_cart.jpg" border="0" name="submit" alt="Make payments with PayPal - it's fast, free and secure!">
